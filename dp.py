...
def value_iteration():
    ...
    for it in range(N_ITERATIONS):
        ...
        for action in ACTION_VALS:
            ...
            ### FILLED: Bellman update - compute expected cost ###
        ...
        ### FILLED: Take minimum cost across all actions ###
        V = np.minimum.reduce(V_new)

        ### FILLED: Check convergence by computing max difference ###
...
def compute_policy(V):
    ### FILLED: Compute Q-values for each action and select best action ###
    Q_values = []
    for action in ACTION_VALS:
        next_states = dynamics(state_tensor, action)
        next_indices = state_to_indices(next_states, [x_vals, x_dot_vals, theta_vals, theta_dot_vals])
        ...
        Q_val = np.where(is_terminal[next_indices], TERMINAL_COST, future_val)
        Q_values.append(Q_val)
    
    Q_values = np.array(Q_values)
    policy = np.argmin(Q_values, axis=0)  # Select action with minimum cost
    return policy
